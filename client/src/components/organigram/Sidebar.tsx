...
// ── Sidebar (admin-stijl) ────────────────────────────────────────────────────
export default function Sidebar({ viewMode, onViewMode, hasTab, isAdmin, username, onLogout, onOpenHelp }: Props) {
  ...
  const sidebarContent = (
    <>
      ...
      <div className="flex-shrink-0 pt-3 border-t border-[rgba(255,255,255,0.08)] mt-auto flex flex-col gap-1">
        {/* Instellingen (tandwiel, admins) + Handleiding (?) */}
        <div className="flex items-center gap-1">
          {isAdmin && (
            <Link
              ...
              className="flex-1 flex items-center justify-center h-9 rounded-[6px] text-[rgba(255,255,255,0.6)] hover:text-white hover:bg-[rgba(255,255,255,0.06)] transition-colors"
            >
          ...
          <button
            type="button"
            onClick={() => { setSidebarOpen(false); onOpenHelp(); }}
            title="Handleiding"
            aria-label="Handleiding"
            className="flex-1 flex items-center justify-center h-9 rounded-[6px] text-[rgba(255,255,255,0.6)] hover:text-white hover:bg-[rgba(255,255,255,0.06)] transition-colors cursor-pointer"
          >
            <svg className="w-[18px] h-[18px]" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
