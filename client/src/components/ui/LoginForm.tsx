...
export default function LoginForm() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    catch { setError('Ongeldige inloggegevens'); }
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-4">
      <div className="bg-[#1a3a38] p-8 rounded-[12px] w-full max-w-sm border border-[rgba(255,255,255,0.08)]">
        ...
        {info && <p className="text-accent-teal text-sm mb-4 text-center">{info}</p>}
        ...
        {mode === 'login' ? (
          <form onSubmit={handleLogin}>
            ...
            <div className="text-right mb-5">
              <button type="button" onClick={() => { setMode('forgot'); setError(''); setInfo(''); }} className="text-[12px] text-[rgba(255,255,255,0.5)] hover:text-accent-teal transition-colors cursor-pointer">
                Wachtwoord vergeten?
              </button>
            </div>
            <button type="submit" disabled={loading} className="w-full py-2.5 rounded-[6px] bg-accent-teal text-[#1a3a38] font-semibold hover:opacity-85 transition-opacity disabled:opacity-50 cursor-pointer">
              {loading ? 'Inloggen…' : 'Inloggen'}
            </button>
          </form>
        ) : (
          <form onSubmit={handleForgot}>
            <p className="text-[rgba(255,255,255,0.6)] text-sm mb-4">Vul je e-mailadres in. Je krijgt een link toegestuurd om een nieuw wachtwoord in te stellen.</p>
            <div className="mb-5">
            ...
            <button type="submit" disabled={loading} className="w-full py-2.5 rounded-[6px] bg-accent-teal text-[#1a3a38] font-semibold hover:opacity-85 transition-opacity disabled:opacity-50 cursor-pointer mb-2">
              {loading ? 'Versturen…' : 'Verstuur reset link'}
            </button>
            <button type="button" onClick={() => { setMode('login'); setError(''); setInfo(''); }} className="w-full text-[12px] text-[rgba(255,255,255,0.5)] hover:text-white py-1 cursor-pointer transition-colors">
              ← Terug naar inloggen
            </button>
          </form>
        )}
      ...
      <p className="fixed bottom-4 left-0 right-0 text-[12px] text-[rgba(255,255,255,0.35)] text-center">
        Built by{' '}
        <a href="mailto:development@heesakkers.com" className="hover:text-accent-teal transition-colors">Niel Heesakkers</a>
        {' · Vibe-coded with '}
        <a href="https://claude.ai" target="_blank" rel="noreferrer" className="hover:text-accent-teal transition-colors">Claude</a>
      </p>
    </div>
  );
}
